...
const Header = ({ profile }) => {
  ...
  return (
    <>
      <header
        ref={headerRef}
        className="bg-gradient-to-br from-blue-100 to-slate-100 min-h-screen flex flex-col justify-center items-center text-center px-4"
      >
      ...
      {isSticky && (
        <nav className="fixed top-0 left-0 right-0 bg-white shadow-md z-50 flex justify-center gap-6 py-3">
          {[
            ...
            <button
              key={section}
              className="text-gray-600 font-medium hover:text-blue-600 capitalize"
              onClick={() => scrollToSection(section)}
            >
              {section}
            </button>
